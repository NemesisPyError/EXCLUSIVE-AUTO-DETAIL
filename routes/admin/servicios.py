...
def listar_servicios():
    lista = Servicio.query.filter_by(deleted_at=None).options(
        joinedload(Servicio.precios),
    ...
    for s in lista:
        if s.precios:
            precios_map[s.id] = min(p.precio for p in s.precios)
            duraciones_map[s.id] = min(p.duracion_minutos for p in s.precios)
        else:
            precios_map[s.id] = 0
            duraciones_map[s.id] = 0
...
def crear_servicio():
    ...
    servicio = Servicio(
        ...
        categoria_servicio_id=int(data.get('categoria_servicio_id') or 1),
